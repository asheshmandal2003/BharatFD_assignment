...
@pytest.fixture
def sample_faq(db):
    return FAQ.objects.create(question='What is your name?', answer='My name is John Doe')

def create_faq(api_client, db):
    response = api_client.post('/api/v1/faqs', {'question': 'Test Question', 'answer': 'Test Answer'}, format='json')
...
def test_cache_behavior(api_client, sample_faq):
    cache.set("faqs_en", [{"id": sample_faq.id, "question": "Cached question?", "answer": "Cached answer"}], timeout=180)
    response = api_client.get('/api/v1/faqs/')
...
def test_faq_cache_expiry(api_client):
    cache.clear()
    faq = FAQ.objects.create(question="Cached question", answer="Cached answer")
    cache.set("faqs_en", [{"id": faq.id, "question": faq.question, "answer": faq.answer}], timeout=5)
